...
export const searchPersonByNameSuccess = (search, persons) => ({
  type: SEARCH_PERSON_SUCCESS,
  search,
  persons
})
export const searchPersonByNameFail = (search, persons) => ({
  type: SEARCH_PERSON_FAIL,
  error: true,
  search,
  persons
})
...
export const searchPersonByName = searchString => async dispatch => {
  try {
    ...
    const result = await api.searchUserByName(searchString)
