  ...
  addPersonToData(e) {
    ...
    const photo =
      this.state.uploadedFileCloudinaryUrl === ""
        ? "https://res.cloudinary.com/csucu/image/upload/v1524057401/av97c7rihdxzy7apnjaj.jpg"
        : this.state.uploadedFileCloudinaryUrl;
    ...
    if (getEditingPersonId(state)) {
      ...
      if (person) {
        motherName =
          person.mother === null
            ? ""
            : getPersonById(person.mother, state).name +
              " " +
              getPersonById(person.mother, state).surname;
        fatherName =
          person.father === null
            ? ""
            : getPersonById(person.father, state).name +
              " " +
              getPersonById(person.father, state).surname;
      }
    }
    return {
      person,
